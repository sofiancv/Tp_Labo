...
paises_objetivo=sql^ """SELECT DISTINCT origen,destino FROM flujos_migratorios
                        INNER JOIN paises_con_sedes_argentinas ON origen=pais_iso_3 
                        WHERE destino IN (SELECT DISTINCT pais_iso_3 FROM paises_con_sedes_argentinas)"""
#Necesitamos sumar todas las emigraciones del año 2000 en cada pais con al menos una sede argentina
flujo_emigratorio_2000=sql^ """SELECT DISTINCT p.origen , 
                          SUM(CAST(fm.casos_2000 AS INTEGER)) AS flujo_emi
                          FROM paises_objetivo AS p 
                          INNER JOIN flujos_migratorios AS fm
                          ON p.origen= fm.origen AND p.destino=fm.destino 
                          GROUP BY p.origen 
                         """
#realizamos lo mismo con las inmigraciones
flujo_inmigratorio_2000=sql^ """SELECT DISTINCT p.destino , 
                          SUM(CAST(fm.casos_2000 AS INTEGER)) AS flujo_inmi
                          FROM paises_objetivo AS p 
                          INNER JOIN flujos_migratorios AS fm
                          ON p.origen= fm.origen AND p.destino=fm.destino 
                          GROUP BY p.destino 
                        """
#Calculamos el promedio
flujo_migratorio_paises_2000=sql^""" SELECT fe.origen, (flujo_emi-flujo_inmi)/5 AS flujo_migratorio 
                                FROM flujo_emigratorio AS fe
                                INNER JOIN flujo_inmigratorio AS fi ON fe.origen=fi.destino"""


#Agregamos la canatidad de sedes

df_ii=sql^""" SELECT se.pais_iso_3 as pais, se.cantidad_de_sedes, (CASE WHEN fm.flujo_migratorio is NULL then 0 else fm.flujo_migratorio END) as flujo_migratorio
            FROM sedes_por_codigo_pais as se
            LEFT JOIN flujo_migratorio_paises_2000 AS fm ON se.pais_iso_3=fm.origen
            ORDER BY fm.flujo_migratorio DESC
            """
        
altura_barras = 0.8
altura = np.empty(len(df_ii))
altura.fill(altura_barras)


fig, ax = plt.subplots(figsize=(8, 16))


ax.barh(data=df_ii, y="pais", width="flujo_migratorio", height=altura)
ax.invert_yaxis() 
ax.set_title('Flujo migratorio')
ax.set_xticks([item for item in range(-15000000,7000000,1000000)])
...
#https://medium.com/@yxs682/add-color-bars-in-python-plt-matplotlib-pyplot-at-will-9a37ba142c59
